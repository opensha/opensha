...
public final class WeibullDistCalc extends EqkProbDistCalc implements ParameterChangeListener {
	...
	public double getCondProbTest(double timeSinceLast, double duration) {
		WeibullDistribution weibullDist = WeibullDistribution.of(k, lambda);
	...
	public static double getShapeParameterOld(double cov) {
		...
		for(int i=0; i<1950; i++) {
			...
			double bigGamma1 = Gamma.value(1d+1d/test_k);
			double bigGamma2 = Gamma.value(1d+2d/test_k);
	...
	public static double getScaleParameter(double mean, double shapeParam) {
		return mean/Gamma.value(1d+1d/shapeParam);

	}


	public static double getCOV_fromShapeParam(double shapeParam) {
		double term1 = Gamma.value(1.0 + 2.0 / shapeParam);
		double term2 = Gamma.value(1.0 + 1.0 / shapeParam);
		return Math.sqrt(term1/(term2*term2) - 1d);
		// Alternative LogGamma version that may be more stable (see note below). Consider using these instead,
		// but I'm not changing them because I don't want current caclculations to change
//		double logGamma1 = LogGamma.value(1d + 1d/shapeParam);
//		double logGamma2 = LogGamma.value(1d + 2d/shapeParam);
//		return Math.sqrt(Math.expm1(logGamma2 - 2d*logGamma1));
	}


	public static double getMean_fromShapeAndScaleParams(double shapeParam, double scaleParam) {
		return scaleParam * Gamma.value(1.0 + 1.0 / shapeParam);
	}
	...
	public static double getShapeParameter(double cov) {
		...
		java.util.function.DoubleUnaryOperator f = (k) -> {
			double term1 = Gamma.value(1.0 + 2.0 / k);
			double term2 = Gamma.value(1.0 + 1.0 / k);
			return (term1 / (term2 * term2)) - 1.0 - cv2;

			// Alternative for very large k (very small CV), or if the search interval is expanded: computing the
			// Gamma ratio in log space avoids overflow in the individual Gamma values and Math.expm1 preserves
			// precision when that ratio is very close to 1. The direct form above is adequate over the current
			// k=[0.1, 20] search interval and is left active for clarity.
			//            double logGamma1 = org.apache.commons.numbers.gamma.LogGamma.value(1.0 + 1.0 / k);
			//            double logGamma2 = org.apache.commons.numbers.gamma.LogGamma.value(1.0 + 2.0 / k);
			//            return Math.expm1(logGamma2 - 2.0 * logGamma1) - cv2;
		};

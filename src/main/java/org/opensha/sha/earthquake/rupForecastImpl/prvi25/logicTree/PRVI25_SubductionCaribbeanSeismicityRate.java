...
//			double magCorner = Double.NaN;
			double magCorner = slab ? 7.4 : Double.NaN;
			EvenlyDiscretizedFunc refMFD = FaultSysTools.initEmptyMFD(5.01, mMax);
			
			EvenlyDiscretizedFunc pref, low, high, avg;
			if (cumulative) {
				pref = PREFFERRED.build(epoch, refMFD, mMax, magCorner, slab).getCumRateDistWithOffset();
				low = LOW.build(epoch, refMFD, mMax, magCorner, slab).getCumRateDistWithOffset();
				high = HIGH.build(epoch, refMFD, mMax, magCorner, slab).getCumRateDistWithOffset();
				avg = AVERAGE.build(epoch, refMFD, mMax, magCorner, slab).getCumRateDistWithOffset();
			} else {
				pref = PREFFERRED.build(epoch, refMFD, mMax, magCorner, slab);
				low = LOW.build(epoch, refMFD, mMax, magCorner, slab);
				high = HIGH.build(epoch, refMFD, mMax, magCorner, slab);
				avg = AVERAGE.build(epoch, refMFD, mMax, magCorner, slab);
			}
			
			for (int i=0; i<refMFD.size(); i++) {
				if (refMFD.getX(i) > refMFD.getClosestXIndex(mMax))
					break;
				System.out.println((float)pref.getX(i)+"\t"+(float)pref.getY(i)+"\t["+(float)low.getY(i)+","+(float)high.getY(i)+"]\tavg="+(float)avg.getY(i));
			}
			System.out.println();
		}
	}
